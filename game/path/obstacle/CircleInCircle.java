...
public class CircleInCircle extends Obstacle {
   ...
   /* public CircleInCircle(double x, double y, List<Color> colors, Difficulty difficulty) {
    ...
    protected Group buildObstacle(ModelObstacle mcic) {
        ...
        int colorOppose = (colorDeb+nb_arc/2)%mcic.getColors().size();
        ...
        switch (mcic.getVersion()) {
            ...
            case 3:
            	...
            	color_passable.add(mcic.getColors().get((colorDeb+1)%mcic.getColors().size()));
            	color_passable.add(mcic.getColors().get((colorOppose+1)%mcic.getColors().size()));
